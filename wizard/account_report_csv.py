...
class AccountCSVExport(orm.TransientModel):
    _name = 'account.report.csv'
    _description = 'Export Accounting Balances'
    ...
    _columns = {
        ...
        'by_currency': fields.boolean('Group by currency', help='When checked, transaction amounts are also exported and grouped by currency'),
    ...
    _defaults = {'company_id': _get_company_default,
                 ...
                 'export_filename': 'account_report.csv',
                 'by_currency': False, }

    def action_export_tbl(self, cr, uid, ids, context=None):
        this = self.browse(cr, uid, ids)[0]
        rows = self.get_data(cr, uid, ids,"tbl", context)
    ...
    def _get_header_tbl(self, cr, uid, ids, context=None):
        res = [_(u'CODE'),
               _(u'NAME'),
               ]

        if context.get('report_csv_by_currency', False):
            res += [_(u'CURRENCY'),
                    _(u'DEBIT'),
                    _(u'CREDIT'),
                    _(u'BALANCE'),
    ...
    def _get_rows_tbl(self, cr, uid, ids, fiscalyear_id,period_range_ids,company_id,context=None):
        """
        ...
        if context.get('report_csv_by_currency', False):
            query = """
                        select ac.code,ac.name,cur.name,
                        sum(debit_curr) as sum_debit_curr,sum(credit_curr) as sum_credit_curr,sum(debit_curr) - sum(credit_curr) as balance_curr,
                        sum(debit) as sum_debit,sum(credit) as sum_credit,sum(debit) - sum(credit) as balance
                        from account_move_line as aml,account_account as ac,res_currency as cur
                        where aml.account_id = ac.id
                        and aml.currency_id = cur.id
                        and period_id in %(period_ids)s
                        group by ac.id,ac.code,ac.name,cur.name
                        order by ac.code
                   """
    ...
    # def action_export_analytic(self, cr, uid, ids, context=None):
    #     this = self.browse(cr, uid, ids)[0]
    ...
    def get_data(self, cr, uid, ids,result_type,context=None):
        ...
        # pass the group by currency toggle in the context
        context['report_csv_by_currency'] = form.by_currency
        rows = []
